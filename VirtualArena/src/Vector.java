...
public class Vector { 
    ...
    public Vector(double... data) {
		...
		public Vector direction() {
			if (this.magnitude() == 0.0f) throw new RuntimeException("Zero-vector has no direction");
		...
		public static float angleBetween(Vector a, Vector b){
			...
			float angle = 0.0f; 
			double dot = a.dot(b);
			double prod = a.magnitude()*b.magnitude();
			angle = (float)PApplet.acos((float)(dot/prod));
			return (float)PApplet.degrees(angle);
		}
